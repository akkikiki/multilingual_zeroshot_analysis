...
from datasets.wikibib_dataset import WikiBibDataset
from datasets.myiterable_dataset import MyIterableDataset
from biblebert.multieval_trainer import MultiEvalTrainer
from biblebert.mycallback import MyCallback
from biblebert.mycallback_epoch10 import MyCallbackEpoch10
...
if __name__ == "__main__":
    ...
    tokenizer = XLMTokenizer.from_pretrained(args.tokenizer_path, max_len=args.max_seq_len, padding='max_length')
    ...
    if args.load_from_checkpoint:
        ...
        model = XLMWithLMHeadModel.from_pretrained(args.load_from_checkpoint)
    else:
        logging.info('Initializing new random XLM model')
        config = XLMConfig.from_pretrained(args.tokenizer_path)
        model = XLMWithLMHeadModel.from_pretrained(config=config)
